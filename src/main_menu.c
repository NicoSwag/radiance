...
static void Task_NewGameBirchSpeech_StartTrainerFadeOut(u8);
static void Task_NewGameBirchSpeech_StartCosmonFadeOut(u8);
static void Task_NewGameBirchSpeech_StartCosmonFadeOutLast(u8);
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8);
static void Task_NewGameBirchSpeech_WaitForPlayerFadeIn(u8);
static void Task_NewGameBirchSpeech_WaitForPlayerFadeIn2(u8);
static void NewGameBirchSpeech_StartFadeOutTarget1InTarget2(u8, u8);
static void Task_NewGameBirchSpeech_StartPlayerFadeIn2(u8);
static void Task_NewGameBirchSpeech_StartPlayerFadeInLast(u8);
static void Task_NewGameBirchSpeech_StartCosmonFadeIn(u8);
static void Task_NewGameBirchSpeech_StartLeoFadeIn(u8);
static void Task_NewGameBirchSpeech_StartCosmonFadeInLast(u8);
static void Task_NewGameBirchSpeech_WaitForIn2(u8);
static void Task_NewGameBirchSpeech_WaitForPlayerFadeInLast(u8);
static void Task_NewGameBirchSpeech_WaitForCosmonFadeIn(u8);
static void Task_NewGameBirchSpeech_WaitForCosmonFadeInLast(u8);
static void Task_NewGameBirchSpeech_WaitForLeoFadeIn(u8);
static void Task_NewGameBirchSpeech_BoyOrGirl(u8);
static void Task_NewGameBirchSpeech_YoungHuman(u8);
static void Task_NewGameBirchSpeech_TwoSouls(u8);
static void Task_NewGameBirchSpeech_ThisIsYourSister(u8);
static void Task_NewGameBirchSpeech_KnowMyself(u8);
static void Task_NewGameBirchSpeech_ThatsWrong(u8);
static void Task_NewGameBirchSpeech_ThatsWrongLeo(u8);
...
static void Task_NewGameBirchSpeech_WaitForWhatsYourNameToPrintLeo(u8);
static void Task_NewGameBirchSpeech_WaitPressBeforeNameChoice(u8);
static void Task_NewGameBirchSpeech_WaitPressBeforeNameChoiceLeo(u8);
static void Task_NewGameBirchSpeech_StartNamingScreen(u8);
static void Task_NewGameBirchSpeech_StartNamingScreenLeo(u8);
static void CB2_NewGameBirchSpeech_ReturnFromNamingScreen(void);
static void CB2_NewGameBirchSpeech_ReturnFromNamingScreenLeo(void);
static void Task_NewGameBirchSpeech_CreateNameYesNo(u8);
static void Task_NewGameBirchSpeech_CreateNameYesNoLeo(u8);
static void Task_NewGameBirchSpeech_ProcessNameYesNoMenu(u8);
static void Task_NewGameBirchSpeech_ProcessNameYesNoMenuLeo(u8);
void CreateYesNoMenuParameterized(u8, u8, u16, u16, u8, u8);
static void Task_NewGameBirchSpeech_StartTrainerFadeOutShowLeo(u8);
static void Task_NewGameBirchSpeech_StartTrainerFadeOutShowCosmonLast(u8);
...
static void Task_NewGameBirchSpeech_ReturnFromNamingScreenShowTextboxLeo(u8);
...
//If I don't do this, the game doesn't compile. I have no clue why and I don't care enough to figure it out.
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInWelcome(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        else
        {
            ...
            LoadMessageBoxGfx(0, 0XA, BG_PLTT_ID(15));
            NewGameBirchSpeech_ShowDialogueWindow(0, 1);
            PutWindowTilemap(0);
            CopyWindowToVram(0, COPYWIN_GFX);
            NewGameBirchSpeech_ClearWindow(0);
            StringExpandPlaceholders(gStringVar4, gText_Intro_AmIDreaming);
            AddTextPrinterForMessageWhite(TRUE);
            gTasks[taskId].func = Task_NewGameBirchSpeech_StartTrainerFadeOut;
        }
    }
}

static void Task_NewGameBirchSpeech_StartTrainerFadeOut(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartCosmonFadeIn;
    }
}


static void Task_NewGameBirchSpeech_StartCosmonFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        else
        {
            ...
            NewGameBirchSpeech_StartFadeInTarget1OutTarget2(taskId, 1);
            gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForCosmonFadeIn;
        }
    }
}

static void Task_NewGameBirchSpeech_WaitForCosmonFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tPlayerSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gTasks[taskId].func = Task_NewGameBirchSpeech_YoungHuman;
    }
}

static void Task_NewGameBirchSpeech_YoungHuman(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Intro_YoungHuman);
    AddTextPrinterForMessageWhite(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_StartCosmonFadeOut;
}


static void Task_NewGameBirchSpeech_StartCosmonFadeOut(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        gTasks[taskId].tTimer = 10;
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartPlayerFadeIn2;
    }
}

static void Task_NewGameBirchSpeech_StartPlayerFadeIn2(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        else
        {
            ...
            gTasks[taskId].tPlayerGender = MALE;
            NewGameBirchSpeech_StartFadeInTarget1OutTarget2(taskId, 1);
            gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForPlayerFadeIn2;
        }
    }
}

static void Task_NewGameBirchSpeech_WaitForPlayerFadeIn2(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tPlayerSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gTasks[taskId].func = Task_NewGameBirchSpeech_KnowMyself;
    }
}

static void Task_NewGameBirchSpeech_KnowMyself(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Intro_KnowMyself);
    AddTextPrinterForMessageWhite(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForWhatsYourNameToPrint;
}


static void Task_NewGameBirchSpeech_ThatsWrong(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Intro_ThatsWrong);
    AddTextPrinterForMessageWhite(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForWhatsYourNameToPrint;
}

static void Task_NewGameBirchSpeech_ThatsWrongLeo(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Intro_ThatsWrongLeo);
    AddTextPrinterForMessageWhite(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForWhatsYourNameToPrintLeo;
}
...
static void Task_NewGameBirchSpeech_SoItsRivalName(u8 taskId)
{
    ...
    AddTextPrinterForMessageWhite(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_CreateNameYesNoLeo;
}
...
static void Task_NewGameBirchSpeech_CreateNameYesNoLeo(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        CreateYesNoMenuParameterized(2, 1, 0xF3, 0xDF, 2, 15);
        gTasks[taskId].func = Task_NewGameBirchSpeech_ProcessNameYesNoMenuLeo;
    }
}
...
static void Task_NewGameBirchSpeech_ProcessNameYesNoMenu(u8 taskId)
{
    switch (Menu_ProcessInputNoWrapClearOnChoose())
    {
        case 0:
            ...
            NewGameBirchSpeech_StartFadeOutTarget1InTarget2(taskId, 0);
            gTasks[taskId].func = Task_NewGameBirchSpeech_StartTrainerFadeOutShowLeo;
            break;
        case MENU_B_PRESSED:
        case 1:
            PlaySE(SE_SELECT);
            gTasks[taskId].func = Task_NewGameBirchSpeech_ThatsWrong;
    }
}

static void Task_NewGameBirchSpeech_ProcessNameYesNoMenuLeo(u8 taskId)
{
    switch (Menu_ProcessInputNoWrapClearOnChoose())
    {
        case 0:
            ...
            gSprites[gTasks[taskId].tPlayerSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            NewGameBirchSpeech_StartFadeOutTarget1InTarget2(taskId, 0);
            gTasks[taskId].func = Task_NewGameBirchSpeech_StartTrainerFadeOutShowCosmonLast;
            break;
        case MENU_B_PRESSED:
        case 1:
            PlaySE(SE_SELECT);
            gTasks[taskId].func = Task_NewGameBirchSpeech_ThatsWrongLeo;
    }
}

static void Task_NewGameBirchSpeech_StartTrainerFadeOutShowLeo(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBrendanSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        NewGameBirchSpeech_StartFadeOutTarget1InTarget2(taskId, 0);
        gTasks[taskId].tTimer = 10;
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartLeoFadeIn;
    }
}

static void Task_NewGameBirchSpeech_StartLeoFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tBrendanSpriteId].invisible = TRUE;
        ...
        else
        {
            ...
            NewGameBirchSpeech_StartFadeInTarget1OutTarget2(taskId, 1);
            gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForLeoFadeIn;
        }
    }
}


static void Task_NewGameBirchSpeech_WaitForLeoFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tPlayerSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gTasks[taskId].func = Task_NewGameBirchSpeech_ThisIsYourSister;
    }
}


static void Task_NewGameBirchSpeech_ThisIsYourSister(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Intro_ThisIsYourSister);
    AddTextPrinterForMessageWhite(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForWhatsYourNameToPrintLeo;
}


static void Task_NewGameBirchSpeech_WaitForWhatsYourNameToPrintLeo(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
        gTasks[taskId].func = Task_NewGameBirchSpeech_WaitPressBeforeNameChoiceLeo;
}

static void Task_NewGameBirchSpeech_WaitPressBeforeNameChoiceLeo(u8 taskId)
{
    if ((JOY_NEW(A_BUTTON)) || (JOY_NEW(B_BUTTON)))
    {
        BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartNamingScreenLeo;
    }
}

static void Task_NewGameBirchSpeech_StartNamingScreenLeo(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        FreeAllWindowBuffers();
        ...
        DestroyTask(taskId);
        DoNamingScreen(NAMING_SCREEN_PLAYER, gSaveBlock2Ptr->rivalName, FEMALE, 0, 0, CB2_NewGameBirchSpeech_ReturnFromNamingScreenLeo);
    }
}


static void Task_NewGameBirchSpeech_StartTrainerFadeOutShowCosmonLast(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        NewGameBirchSpeech_StartFadeOutTarget1InTarget2(taskId, 0);
        gTasks[taskId].tTimer = 10;
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartCosmonFadeInLast;
    }
}

static void Task_NewGameBirchSpeech_StartCosmonFadeInLast(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        else
        {
            ...
            gTasks[taskId].tPlayerGender = MALE;
            NewGameBirchSpeech_StartFadeInTarget1OutTarget2(taskId, 1);
            gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForCosmonFadeInLast;
        }
    }
}

static void Task_NewGameBirchSpeech_WaitForCosmonFadeInLast(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tPlayerSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gTasks[taskId].func = Task_NewGameBirchSpeech_TwoSouls;
    }
}

static void Task_NewGameBirchSpeech_TwoSouls(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Intro_TwoSouls);
    AddTextPrinterForMessageWhite(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_StartCosmonFadeOutLast;
}

static void Task_NewGameBirchSpeech_StartCosmonFadeOutLast(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        NewGameBirchSpeech_StartFadeOutTarget1InTarget2(taskId, 0);
        gTasks[taskId].tTimer = 10;
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartPlayerFadeInLast;
    }
}

static void Task_NewGameBirchSpeech_StartPlayerFadeInLast(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tBirchSpriteId].invisible = TRUE;
        ...
        else
        {
            ...
            gTasks[taskId].tPlayerGender = MALE;
            NewGameBirchSpeech_StartFadeInTarget1OutTarget2(taskId, 1);
            gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForPlayerFadeInLast;
        }
    }
}

static void Task_NewGameBirchSpeech_WaitForPlayerFadeInLast(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tPlayerSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
...
static void CB2_NewGameBirchSpeech_ReturnFromNamingScreenLeo(void)
{
    ...
    LoadPalette(&sBirchSpeechBgGradientPal[1], BG_PLTT_ID(0) + 1, PLTT_SIZEOF(8));
    ResetTasks();
    taskId = CreateTask(Task_NewGameBirchSpeech_ReturnFromNamingScreenShowTextboxLeo, 0);
    gTasks[taskId].tTimer = 10;
    ScanlineEffect_Stop();
    ResetSpriteData();
    FreeAllSpritePalettes();
...
// Note: the line was SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_TGT2_BG1 | BLDCNT_EFFECT_BLEND | BLDCNT_TGT1_OBJ);
// The problem is that doing that, it fades in the legs way later. Idk why.
...
static void NewGameBirchSpeech_StartFadeOutTarget1InTarget2(u8 taskId, u8 delay)
{
    ...
    SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_EFFECT_BLEND | BLDCNT_TGT1_OBJ);
...
static void NewGameBirchSpeech_ClearWindow(u8 windowId)
{
    ...
    FillWindowPixelRect(windowId, 0, 0, 0, maxCharWidth * winWidth, maxCharHeight * winHeight);
...
static void Task_NewGameBirchSpeech_ReturnFromNamingScreenShowTextboxLeo(u8 taskId)
{
    if (gTasks[taskId].tTimer-- <= 0)
    {
        NewGameBirchSpeech_ShowDialogueWindow(0, 1);
