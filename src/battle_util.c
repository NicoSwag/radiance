                ...
                for (i = 0; i < MAX_BATTLERS_COUNT; i++)
                {
                    if (IsBattlerAlive(i) && side != GetBattlerSide(i))
                    {
                        for (j = 0; j < MAX_MON_MOVES; j++)
                        {
                            ...
                            moveType = GetBattleMoveType(move);
                            if (CalcTypeEffectivenessMultiplier(move, moveType, i, battler, ABILITY_ANTICIPATION, FALSE) >= UQ_4_12(1.5) || GetMoveEffect(move) == EFFECT_OHKO)
                            {
                                effect++;
                                break;
                            }
...
static inline u32 CalcMoveBasePower(struct DamageCalculationData *damageCalcData, u32 abilityDef, u32 weather)
{
    ...
    case EFFECT_DOUBLE_POWER_ON_ARG_STATUS:
        ...
        if((weather & B_WEATHER_MOON) && !(GetMoveAdditionalEffectById(move, 0)->moveEffect == MOVE_EFFECT_REMOVE_STATUS))
            basePower *= 2;  
        else if ((gBattleMons[battlerDef].status1 | (STATUS1_SLEEP * (abilityDef == ABILITY_COMATOSE))) & GetMoveEffectArg_Status(move)
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    case EFFECT_SOLAR_BEAM:
        if (IsBattlerWeatherAffected(battlerAtk, (B_WEATHER_HAIL | B_WEATHER_SANDSTORM | B_WEATHER_RAIN | B_WEATHER_SNOW | B_WEATHER_MOON | B_WEATHER_FOG)))
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
        break;
    case EFFECT_BLOOD_MOON:
        if (IsBattlerWeatherAffected(battlerAtk, (B_WEATHER_SUN | B_WEATHER_HAIL | B_WEATHER_SANDSTORM | B_WEATHER_RAIN | B_WEATHER_SNOW | B_WEATHER_FOG)))
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
        break;
    ...
    case ABILITY_STRIKER:
        if (IsBallisticMove(move) || IsKickingMove(move))
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        break;
    case ABILITY_WATER_BUBBLE:
        if (moveType == TYPE_WATER)
...
{
    return 3*(((100 + userFinalAttack + (15 * level)) * power) / (targetFinalDefense + 25 ) / 5)/5;
}
